...
public class PersonAddressFromCustomStream {
	...
	public KStream<String, String> kstreamPersonAddressFromCustom(StreamsBuilder builder) {
		KStream<String, String> sourceStream = builder
				.stream("t-person-address-custom", Consumed.with(Serdes.String(), Serdes.String()));
	...
	private KeyValueMapper<String, String, Iterable<KeyValue<String, String>>> expandPersonAddress() {
		return (key, value) -> {
			...
			try {
				PersonMessage original = objectMapper.readValue(value, PersonMessage.class);

				for (AddressMessage address : original.getAddresses()) {
					String targetKey = generateTargetKey(address.getAddressId());
					String targetValue = generateTargetValue(original, address);
	...
	private String generateTargetKey(int addressId) throws JsonProcessingException {
		KafkaConnectMessage<Integer> targetKey = new KafkaConnectMessage<>();
