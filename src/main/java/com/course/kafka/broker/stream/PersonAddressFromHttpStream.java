...
public class PersonAddressFromHttpStream {
	...
	public KStream<String, String> kstreamPersonAddressFromHttp(StreamsBuilder builder) {
		KStream<String, String> sourceStream = builder
				.stream("t-person-address-http", Consumed.with(Serdes.String(), Serdes.String()));
		KStream<String, String> targetStream = sourceStream.flatMap(expandPersonAddress());
	...
	private KeyValueMapper<String, String, Iterable<KeyValue<String, String>>> expandPersonAddress() {
		return (key, value) -> {
			List expanded = new ArrayList<KeyValue<String, String>>();

			try {
				KafkaConnectMessage<KafkaConnectPersonAddressFromHttpMessage> original =
						objectMapper.readValue(value,
							new TypeReference<KafkaConnectMessage<KafkaConnectPersonAddressFromHttpMessage>>() {
						});
				String personAddressJsonString = original.getPayload().getValue();
				KafkaConnectPersonMessageSnakeCase personAddressObject = objectMapper.readValue(personAddressJsonString,
						KafkaConnectPersonMessageSnakeCase.class);
				
				for (KafkaConnectAddressMessageSnakeCase address : personAddressObject.getAddresses()) {
					String targetKey = generateTargetKey(address.getAddressId());
					String targetValue = generateTargetValue(personAddressObject, address);
	...
	private String generateTargetKey(int addressId) throws JsonProcessingException {
		KafkaConnectMessage targetKey = new KafkaConnectMessage<Integer>();
	...
	private String generateTargetValue(KafkaConnectPersonMessageSnakeCase personAddressObject,
		...
		KafkaConnectPersonTargetMessage targetPayload = new KafkaConnectPersonTargetMessage();
		...
		KafkaConnectMessage targetValue = new KafkaConnectMessage<KafkaConnectPersonTargetMessage>();
